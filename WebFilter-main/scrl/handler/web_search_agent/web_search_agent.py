...
class WebSearchAgent:
    def __init__(self, client, config):
        ...
        if not os.path.exists(downloads_folder_path):
            logging.info(f"Download folder does not exist, creating directory: {os.path.abspath(downloads_folder_path)}")
    ...
    def browser_to_json(self, browser: SimpleTextBrowser, title: str, url: str):
        ...
        while browser.viewport_current_page < total_pages:
            ...
            browser.page_down()
        # Extract browser content, convert it into JSON, and save it into the database
        return {
            "title": title,
            "url": url,
            "page_list": page_list
    ...
    def scrape(self, browser, url: str) -> str:
        """Scrape the webpage and use LLM to summarize its content"""
    ...
    def scrape_and_check_valid(self, web_info: dict, user_query, search_query):
        ...
        if self.is_error_page(browser):
            logging.info(f"Error accessing URL, discarding: {web_info['link']}")
            return None
    ...
    def scrape_and_check_valid_api(self, url):
        ...
        if self.is_error_page(browser):
            logging.info(f"Error accessing URL, discarding: {url}")
            return None
        return browser
