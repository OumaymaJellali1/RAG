...
class RayPPOTrainer(object):
    ...
    def _save_checkpoint(self, is_best = False):
        ...
        if is_best:
            None if self.config.trainer.default_hdfs_dir is None else os.path.join(
        ...
        if self.use_critic:
            ...
            if is_best:
                critic_remote_path = None if self.config.trainer.default_hdfs_dir is None else os.path.join(
                self.config.trainer.default_hdfs_dir, f'Best', 'critic')
    ...
    def fit(self):
        ...
        for epoch in range(self.config.trainer.total_epochs):
            for batch_dict in self.train_dataloader:
                ...
                # metrics modify start
                """ Save validation metrics locally """
                save_dir_metrics = f"./outputs/{self.config.trainer.project_name}/{self.config.trainer.experiment_name}"
                os.makedirs(save_dir_metrics, exist_ok=True)
                filename_metrics = os.path.join(save_dir_metrics, f"{self.config.trainer.project_name}.jsonl")
                
                data_to_save_metrics = {
                    "global_step": self.global_steps,
                    "metrics": metrics
                }

                try:
                    with open (filename_metrics, "a", encoding='utf-8') as f:
                        json_line = json.dumps(data_to_save_metrics)
                        f.write(json_line + '\n')
                except Exception as e:
                    print(f"Warning: Failed to write initial validation metrics to {filename_metrics}: {e}")  
                # save besst
                if metrics["val/test_score/Bamboogle_val_f1"] > self.best_score:
                    self._save_checkpoint(is_best = True)
                    self.best_score = metrics["val/test_score/Bamboogle_val_f1"]
